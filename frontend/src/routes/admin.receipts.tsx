...
export const Route = createFileRoute("/admin/receipts")({ component: Page });
...
function Page() {
  ...
  const { data: shops = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<Array<{ _id: string; name: string }>>;
    },
  });

  const { data, isLoading } = useQuery({
    queryKey: ["admin-sales", range.startDate ?? "", range.endDate ?? "", shopFilter],
    queryFn: async () => {
      const params = new URLSearchParams({ limit: "100", page: "1" });
      if (range.startDate) params.set("startDate", range.startDate);
      if (range.endDate) params.set("endDate", range.endDate);
      if (shopFilter) params.set("shopId", shopFilter);
      const res = await fetch(`${API_URL}/pos/sales/all?${params}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
  ...
  const pdfForAdmin = (s: ReceiptSale) =>
    downloadReceiptPdf(s, {
      shopName: shopLabel(s),
      shopLocation: null,
      cashier: null,
    });
    ...
    <div className="space-y-6">
      <header>
        <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-3">
          <ClipboardList className="h-7 w-7 text-primary" /> Receipt log
        </h1>
        <p className="text-muted-foreground">
          All branch sales from the POS. Preview layout matches branch receipts.
        </p>
      </header>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="h-10 rounded-full border border-input bg-background px-4 text-sm min-w-[200px]"
            >
            ...
            <tbody>
              {sales.map((r) => (
                <tr key={r._id} className="border-t border-border">
                  ...
                  <td className="p-4 font-semibold">{shopLabel(r)}</td>
      ...
      <ReceiptBillDialog
        ...
        sale={preview}
        subtitle={
          preview ? `${shopLabel(preview)} · ${new Date(preview.createdAt).toLocaleString()}` : null
        }
        onDownloadPdf={pdfForAdmin}
      />
    </div>
  );
}
